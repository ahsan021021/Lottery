...
export const TicketProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchPendingTickets = async () => {
    try {
      const response = await fetch('http://82.180.137.7:5001/admin/pending-tickets');
      if (!response.ok) {
  ...
  const purchaseTicket = async (ticketData: FormData) => {
    try {
      const response = await fetch('http://82.180.137.7:5001/ticket/purchase', {
        method: 'POST',
        body: ticketData,
      });
  ...
  const verifyTicket = async (id: string) => {
    try {
      const response = await fetch(`http://82.180.137.7:5001/admin/verify-ticket/${id}`, {
        method: 'PATCH',
      });
  ...
  const rejectTicket = async (id: string) => {
    try {
      const response = await fetch(`http://82.180.137.7:5001/admin/reject-ticket/${id}`, {
        method: 'DELETE',
      });
      if (!response.ok) {
  ...
  const fetchVerifiedTickets = async () => {
    try {
      const response = await fetch('http://82.180.137.7:5001/ticket/verified-tickets');
      if (!response.ok) {
  ...
  const getRandomWinners = async (count: number) => {
    try {
      const response = await fetch(`http://82.180.137.7:5001/ticket/random-winners?count=${count}`);
      if (!response.ok) {
  ...
  const selectFinalWinner = async (finalists: Ticket[]): Promise<Ticket | null> => {
    try {
      const response = await fetch('http://82.180.137.7:5001/ticket/select-final-winner', {
        method: 'POST',
        headers: {
  ...
  const issueManualTicket = async (ticketData: { name: string; email: string; nic: string }) => {
    try {
      const response = await fetch('http://82.180.137.7:5001/admin/issue-manual-ticket', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
